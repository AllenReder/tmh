...
release_require_commands grep node npm tar

if tar -tzf "$package" | grep -Eq '^package/(shell(/.*)?|(.*/)?tmha(\.mjs)?|(.*/)?tmh\.zsh)$'; then
  release_fail "npm package contains a legacy tmha or standalone shell-integration file"
fi
...
[ ! -e "$install_root/node_modules/.bin/tmha" ] || release_fail "npm package unexpectedly installed legacy tmha"
